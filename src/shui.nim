import shui/[elements, widgets]

export elements, widgets

when isMainModule:
  import pixie

  converter toPixie*(c: elements.Color): pixie.Color =
    pixie.color(c.r, c.g, c.b, c.a)

  var ui = UI()

  var image = newImage(640, 480)
  image.fill(pixie.color(0.0, 0.0, 0.0, 1.0))
  ...
  ui.onDraw = proc(w: Elem) =
    ctx.beginPath()
    ctx.fillStyle = w.style.bg.toPixie
    ...
    if w.border > 0:
      ctx.strokeStyle = w.style.border.toPixie
    ...
    if w.text.len > 0:
      ctx.fillStyle = w.style.fg.toPixie
