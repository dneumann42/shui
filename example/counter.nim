...
""", boxOpts()):
      card("root.hero", BorderedPanel, boxOpts(spacing = 4, padding = uniformSides(10), align = AlignStretch)):
        cardHeader("root.hero.header"):
          discard ui.text("title", "Shui Counter Dashboard", prefSize = size(480, 30), alignSelf = SelfCenter)
        cardBody("root.hero.body", boxOpts(spacing = 4, padding = uniformSides(6), align = AlignCenter)):
          discard ui.text("subtitle", "Control element hover + card layout stress test", prefSize = size(480, 24), alignSelf = SelfCenter)

      card("root.left", FilledPanel, boxOpts(spacing = 8, padding = uniformSides(10), align = AlignStretch)):
        cardHeader("left.controls.header"):
          discard ui.text("controls.title", "Controls", prefSize = size(180, 24))
        cardBody("left.controls.body", boxOpts(spacing = 6, padding = uniformSides(6), align = AlignStretch)):
          discard ui.pushButton("inc", "Increment", prefSize = size(180, 36), measure = fixed(180, 36))
          discard ui.pushButton("dec", "Decrement", prefSize = size(180, 36), measure = fixed(180, 36))
          discard ui.pushButton("reset", "Reset", prefSize = size(180, 36), measure = fixed(180, 36))
          discard ui.comboBox("step.combo", @["1", "2", "5", "10", "20", "40", "80", "160"], selectedIndex = 0, width = 180, itemHeight = 30)
        cardFooter("left.controls.footer"):
          discard ui.text("step.label", "Step", prefSize = size(60, 22))
          discard ui.text("step.value", "1", prefSize = size(40, 22), alignSelf = SelfEnd)
          discard ui.pushButton("open.dialog", "About", prefSize = size(84, 28))

      card("root.middle", BorderedPanel, boxOpts(spacing = 8, padding = uniformSides(10), align = AlignStretch)):
        cardHeader("middle.metrics.header"):
          discard ui.text("metrics.title", "Metrics", prefSize = size(220, 24), alignSelf = SelfCenter)
        cardBody("middle.metrics.body", boxOpts(spacing = 10, padding = uniformSides(8), align = AlignStretch, expand = true, flex = 1)):
          panel("counter.panel", FilledPanel, boxOpts(spacing = 4, padding = uniformSides(10), align = AlignCenter)):
          ...
          hbox("derived.row", boxOpts(spacing = 8, align = AlignStretch)):
            panel("double.panel", BorderedPanel, boxOpts(spacing = 4, padding = uniformSides(8), prefSize = size(180, 120), expand = true, flex = 1)):
              discard ui.text("double.label", "Double", prefSize = size(80, 18))
              discard ui.text("double.value", "0", prefSize = size(80, 28))
            panel("square.panel", BorderedPanel, boxOpts(spacing = 4, padding = uniformSides(8), prefSize = size(180, 120), expand = true, flex = 1)):
              discard ui.text("square.label", "Square", prefSize = size(80, 18))
              discard ui.text("square.value", "0", prefSize = size(80, 28))

          panel("vscroll.panel", BorderedPanel, boxOpts(spacing = 4, padding = uniformSides(6), prefSize = size(0, 180), align = AlignStretch, expand = true, flex = 1)):
            discard ui.text("vscroll.title", "Vertical Scroll List", prefSize = size(200, 20))
            scrollV("vscroll.list", viewportOpts = boxOpts(prefSize = size(0, 140), expand = true, flex = 1, align = AlignStretch), contentOpts = boxOpts(spacing = 4, padding = uniformSides(4), align = AlignStretch)):
              for i in 0 .. 24:
                discard ui.pushButton("v.item." & $i, "Item " & $(i + 1), prefSize = size(180, 26))

          panel("hscroll.panel", BorderedPanel, boxOpts(spacing = 4, padding = uniformSides(6), prefSize = size(0, 160), align = AlignStretch)):
            discard ui.text("hscroll.title", "Horizontal Scroll List", prefSize = size(220, 20))
            scrollH("hscroll.list", viewportOpts = boxOpts(prefSize = size(0, 112), align = AlignStretch), contentOpts = boxOpts(spacing = 6, padding = uniformSides(4), align = AlignCenter)):
              for i in 0 .. 15:
                discard ui.pushButton("h.item." & $i, "Card " & $(i + 1), prefSize = size(120, 72))

      card("root.right", FilledPanel, boxOpts(spacing = 8, padding = uniformSides(10), align = AlignStretch)):
        cardHeader("right.activity.header"):
          discard ui.text("activity.title", "Activity", prefSize = size(180, 24))
        cardBody("right.activity.body", boxOpts(spacing = 6, padding = uniformSides(6), align = AlignStretch)):
      ...
      dialog("about.dialog", title = "About Shui", showHeader = true, showClose = true, opts = boxOpts(prefSize = size(520, 280), spacing = 8, padding = uniformSides(8))):
        vbox("about.dialog.body.top", boxOpts(spacing = 8, align = AlignStretch)):
        ...
        dialogFooter("about.dialog.footer", boxOpts(justify = SpaceBetween, align = AlignCenter, padding = uniformSides(6))):
          discard ui.text("about.state", "Dialog is open", prefSize = size(180, 24))
          discard ui.pushButton("about.dismiss", "Dismiss", prefSize = size(96, 30))
